...
from transformers import BertTokenizer # BertModel is not directly used by EncoderDecoder shown
...
def run_pipeline(args):
    ...
    # Text Encoder
    text_encoder_module = TextEncoder(d_embedding=768, vocab_size=bert_tokenizer_q.vocab_size, max_len=max_question_length)
...
def eval_pipeline(args):
    ...
    text_encoder_module = TextEncoder(
        ...
        vocab_size=max(bert_tokenizer_q.get_vocab().values()) , 
    ...
    for name, config in datasets_to_evaluate_configs.items():
        ...
        if os.path.exists(json_path):
            ...
            dataloaders_for_eval[name] = DataLoader(
                ...
                pin_memory=True, 
                num_workers=1, # Use num_workers from args
